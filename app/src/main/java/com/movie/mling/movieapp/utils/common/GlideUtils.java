...
public class GlideUtils {
    ...
    public void LoadContextCircleBitmap(Context context, String path, ImageView imageView, int defe) {
        ...
        if (!TextUtils.isEmpty(path) && path.startsWith("http://")) {
            if (context != null && imageView != null)
                if (Util.isOnMainThread()) {
                    Glide.with(context).load(path).placeholder(R.mipmap.icon_user_default_image).error(defe).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
                }
        } else {
            if (context != null && imageView != null) {
                if (Util.isOnMainThread()) {
                    Glide.with(context).load(url).placeholder(R.mipmap.icon_user_default_image).error(defe).bitmapTransform(new GlideCircleTransform(context)).into(imageView);
                }
            }
        }
    }
    ...
    public void LoadfragmentCircleBitmap(Fragment fragment, String path, ImageView imageView, int defe) {
        ...
        if (fragment != null) {
            if (!TextUtils.isEmpty(path) && path.startsWith("http://")) {
                Glide.with(fragment).load(path).placeholder(R.mipmap.icon_user_default_image).error(R.mipmap.icon_user_default_image).bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
            } else {
                Glide.with(fragment).load(url).placeholder(R.mipmap.icon_user_default_image).error(R.mipmap.icon_user_default_image).bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
            }
        }
    }
